...
function calculateMission(event) {
    ...
    if(points <= 0 || isNaN(points)) {
        alert("Por favor, selecione uma missão concluída")
        return
    }

    messageText.innerText = "Missão concluída na SoulUp! A GAIA liberou pontos extras para você."
    pointsText.innerText = "+" + points + " pontos extras"
...
function buyItem(event) {
    ...
    if(userPoints <= 0 || isNaN(userPoints)) {
        alert("Por favor, digite seus pontos extras")
        return
    }
    ...
    if(userPoints >= itemPrice) {
        ...
        messageText.innerText = "Acessório comprado com sucesso para personalizar a GAIA!"
        pointsText.innerText = "Pontos extras restantes: " + total
    } else {
        messageText.innerText = "Você ainda não tem pontos extras suficientes."
        pointsText.innerText = "Faltam " + (itemPrice - userPoints) + " pontos extras"
    }
